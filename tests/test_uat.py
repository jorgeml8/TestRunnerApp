...
def run_test():
    ...
    for uat_config in uat_configs:
        logging.info(f"Starting test on{uat_config['url']} - {uat_config['comment']}")
        results.append(f"Starting test on{uat_config['url']} - {uat_config['comment']}")
        ...
        try:
            ...
            logging.info(f"Test successfully completed for {uat_config['url']}")
            results.append(f"Test successfully completed for {uat_config['url']}")

        except Exception as e:
            error_message = f"Error during the test on {uat_config['url']}: {e}"
            logging.error(error_message)
            results.append(error_message)
